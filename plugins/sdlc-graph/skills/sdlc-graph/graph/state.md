...
| `context.standards_handshake` | object \| null | Frontend/both only. **A coding-standards skill may run its own approval handshake** — one that ends in *"never start the actual work until the user has approved"* — and the graph does not get to ignore another skill's approval rule. So it is settled once at `STRATEGY` and supplied to `IMPLEMENT`, which therefore never stops to re-ask inside a node declared non-interactive. `null` for a backend stack. |
